...
def create_datasets(original_dataset_path,
    ...
    for i, (train_indices, val_indices) in enumerate(kf.split(indices)):
        train_images = [original_images[idx] for idx in train_indices]
        val_images = [original_images[idx] for idx in val_indices]
...
def exposure(image):
    ...
    # Adjust the exposure
    exposure_factor = 2.5  # Increase or decrease this value to adjust the exposure
...
if __name__ == '__main__':
    ...
    #group 1
    # create_datasets(names1,augmentation_transforms=[add_noise,exposure,transforms.GaussianBlur(kernel_size=5, sigma=(0.1, 5)),
    #                                                transforms.RandomRotation(degrees=(10, 20))],flip_transforms=[])
    #
    ...
    #                 flip_transforms=flip_transform,together_transforms=[],together_class=[] )

    # names3 = ["augments_group_3_1", "augments_group_3_2", "augments_group_3_3"]
    #
    # aug = transforms.Compose([transforms.GaussianBlur(kernel_size=7, sigma=(0.1, 5)),transforms.RandomRotation(degrees=30)])
    # together_transform = [transforms.RandomHorizontalFlip(1),add_noise]
    # create_datasets(names3,augmentation_transforms=[aug],flip_transforms=[],
    #                 together_transforms=together_transform,together_class=["i","ii","iii","v","x"])

    names4 = ["augments_group_4_1", "augments_group_4_2", "augments_group_4_3"]
    #group 4 includes flipping iv to vi and vi to iv , and v to v
    #noise is set to 0.1
    ...
    create_datasets(original_dataset_path=original_dataset_path,dataset_names=names4,
                    augmentation_transforms=[aug,transforms.GaussianBlur(kernel_size=9, sigma=(0.1, 5)),add_noise,transforms.RandomRotation(degrees=15)],
                    flip_transforms=flip_transform,
                    together_transforms=[],
                    together_class=[],flip_v=True)
